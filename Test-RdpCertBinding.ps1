...
function Test-RdpCertBinding {
    Write-Information "=== 开始 RDP 证书绑定专项测试 ===" 

    # 1. 测试非管理员权限校验
    Write-Information "`n[Test 1] 非管理员权限校验" 
    $currentPrincipal = New-Object Security.Principal.WindowsPrincipal([Security.Principal.WindowsIdentity]::GetCurrent())
    $isAdmin = $currentPrincipal.IsInRole([Security.Principal.WindowsBuiltInRole]::Administrator)
    if (-not $isAdmin) {
        Write-Information "✅ 非管理员，符合预期，无法修改HKLM" 
    }
    else {
        Write-Information "ℹ️ 当前已以管理员身份运行" 
    }

    # 2. 生成测试用自签名证书
    Write-Information "`n[Test 2] 生成测试证书" 
    ...
    Write-Information "✅ 测试证书生成：$($testCert.Thumbprint)" 

    # 3. 测试指纹转字节数组
    Write-Information "`n[Test 3] 指纹转字节数组验证" 
    ...
    Write-Information "转换后字节数组长度：$($bytes.Length)（预期20）" 
    if ($bytes.Length -ne 20) { throw "字节数组长度错误" }

    # 4. 管理员环境下测试注册表写入
    if ($isAdmin) {
        Write-Information "`n[Test 4] 测试注册表写入（临时项）" 
        ...
        Write-Information "读取值类型：$readType（预期Byte[]）" 
        
        if ($readType -ne "Byte[]") { throw "注册表类型错误" }

        # 清理测试项
        Remove-ItemProperty -Path $rdpRegPath -Name $testRegName -Force -ErrorAction SilentlyContinue
        Write-Information "✅ 测试注册表项已清理" 
    }

    Write-Information "`n[Test 5] 函数 Set-RdpCertBinding 逻辑校验" 
    if ($isAdmin) {
        try {
            if ($testCert.HasPrivateKey) {
                Write-Information "✅ 证书包含私钥，校验通过" 
            }
        }
        catch {
            Write-Error "❌ 函数校验失败：$_" 
            throw
        }
    }
    ...
    Write-Information "`n✅ 测试证书已清理" 

    Write-Information "`n=== 所有测试通过 ===" 
}
